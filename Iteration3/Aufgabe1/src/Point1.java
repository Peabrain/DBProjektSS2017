...
public class Point1 extends java.awt.Point{
 
    /**
	 * 
	 */
	private static final long serialVersionUID = -3977856783781240923L;
	/**
	 * 
	 */
	private double x = 0;
  ...
  //Erweitert so das jeder Punkt auch ein Name hat (bzw. Hashtag)  
    ...
    protected static double distance(Point1 p, Point1 centroid) {
        return Math.sqrt(Math.pow((centroid.getY() - p.getY()), 2) + Math.pow((centroid.getX() - p.getX()), 2));
    }
    
    //Creates random point
    protected static Point1 createRandomPoint(int min, int max) {
    	Random r = new Random();
